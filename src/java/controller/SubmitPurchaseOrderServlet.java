                ...
                for (int i = 0; i < productCodes.length; i++) {
                    if (productCodes[i] != null && !productCodes[i].trim().isEmpty()) {
                        ...
                        if (pricesPerUnit != null && i < pricesPerUnit.length && pricesPerUnit[i] != null) {
                            try {
                                String priceStr = pricesPerUnit[i].trim();

                                // Xử lý định dạng Việt Nam: loại bỏ dấu chấm phân cách hàng nghìn
                                // và chuyển dấu phẩy thập phân thành dấu chấm
                                priceStr = priceStr.replaceAll("\\.", ""); // Loại bỏ dấu chấm phân cách hàng nghìn
                                priceStr = priceStr.replace(",", "."); // Chuyển dấu phẩy thập phân thành dấu chấm
                                ...
                                System.out.println("✅ Parsed price: " + pricesPerUnit[i] + " → " + pricePerUnit);
...
// Tương tự cho total price
                        BigDecimal totalPrice = BigDecimal.ZERO;
                        if (totalPrices != null && i < totalPrices.length && totalPrices[i] != null) {
                            try {
                                String totalStr = totalPrices[i].trim();

                                // Xử lý định dạng Việt Nam
                                totalStr = totalStr.replaceAll("\\.", ""); // Loại bỏ dấu chấm phân cách hàng nghìn
                                totalStr = totalStr.replace(",", "."); // Chuyển dấu phẩy thập phân thành dấu chấm
                                ...
                                System.out.println("✅ Parsed total: " + totalPrices[i] + " → " + totalPrice);
                            } catch (NumberFormatException e) {
