...
public class GetStatusOfPurchaseRequestInformationDAO {
     ...
     */
    // Cập nhật method getAllPurchaseRequests để lấy reject_reason
    public List<Request> getAllPurchaseRequests(int index, String startDateStr, String endDateStr, String statusFilter, String requestIdFilter) {
        ...
        try {
            ...
            // BƯỚC 2: Lấy chi tiết đầy đủ cho các request_id đã lọc (bao gồm reject_reason)
            StringBuilder sqlDetails = new StringBuilder(
                    "SELECT r.id AS request_id, r.fullname, r.role, r.day_request, r.status, r.reason, r.reject_reason, "
                ...
                try (ResultSet rs = stmt.executeQuery()) {
                    ...
                    while (rs.next()) {
                        ...
                        if (currentRequest == null || !currentRequest.getId().equals(requestId)) {
                            ...
                            currentRequest.setReject_reason(rs.getString("reject_reason")); // Thêm dòng này
    ...
    // Thêm method mới để update reject status với lý do
    public boolean updateRejectedStatusWithReason(String requestId, String rejectReason) {
        if (requestId == null || requestId.trim().isEmpty()) {
            return false;
        }

        String sql = "UPDATE request SET status = ?, reject_reason = ? WHERE id = ?";
        try (Connection con = Context.getJDBCConnection(); PreparedStatement stmt = con.prepareStatement(sql)) {
            con.setAutoCommit(false);
            stmt.setString(1, "rejected");
