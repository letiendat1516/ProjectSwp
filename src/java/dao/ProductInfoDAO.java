...
public ProductInfo getProductById(int productId) {
    ...
    if (hasMinStockThreshold) {
        sql = "SELECT p.id, p.name, p.code, p.cate_id, p.unit_id, p.status, p.description, " +
              ...
              "COALESCE(ps.qty, 0) as stock_qty " +  // ✅ Sửa: ps.qty thay vì p.qty
              "FROM product_info p " +
              "LEFT JOIN product_in_stock ps ON p.id = ps.product_id " +  // ✅ Thêm JOIN
              "WHERE p.id = ?";
    } else {
        sql = "SELECT p.id, p.name, p.code, p.cate_id, p.unit_id, p.status, p.description, " +
              "COALESCE(ps.qty, 0) as stock_qty " +  // ✅ Sửa: ps.qty thay vì p.qty
    ...
    try (Connection con = Context.getJDBCConnection(); 
        ...
        try (ResultSet rs = stmt.executeQuery()) {
            if (rs.next()) {
                ...
                // Set stock quantity from product_in_stock table
