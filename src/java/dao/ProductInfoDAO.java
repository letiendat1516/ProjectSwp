...
public class ProductInfoDAO {
    ...
    public List<ProductInfo> getAllProducts() {
        ...
        String sql = "SELECT id, name, code, unit_id FROM product_info WHERE active_flag = 1 ORDER BY name";
        ...
        try (Connection con = Context.getJDBCConnection(); PreparedStatement stmt = con.prepareStatement(sql); ResultSet rs = stmt.executeQuery()) {
            ...
            while (rs.next()) {
                ...
                // Debug log
                System.out.println("Loaded product: " + product.getName() + ", Unit ID: " + product.getUnit_id());
            }
    ...
    public boolean updateProductStock(int productId, double newQuantity) {
        ...
        try (Connection con = Context.getJDBCConnection(); PreparedStatement updateStmt = con.prepareStatement(updateSql)) {
    ...
    /**
     * Check if product code exists for other products (for duplicate validation
     * during update)
     */
