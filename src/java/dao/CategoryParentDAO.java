...
public class CategoryParentDAO {
  ...
  /**
   * Update parent category - UPDATED to handle child categories
   */
  public boolean updateCategoryParent(CategoryProductParent category) {
      ...
      try {
          ...
          String updateParentSql = "UPDATE category_parent SET name = ?, description = ?, active_flag = ? WHERE id = ?";
          ...
          try (PreparedStatement ps = conn.prepareStatement(updateParentSql)) {
              ...
              if (rowsAffected > 0) {
                  // 3. If parent is being deactivated, deactivate all child categories
                  if (!category.isActiveFlag()) {
                      boolean childrenUpdated = deactivateChildCategories(conn, category.getId());
                      if (!childrenUpdated) {
                          conn.rollback();
                          System.out.println("Failed to update child categories");
                          return false;
                      }
                  }
                  ...
                  // Commit transaction
                  conn.commit();
                  System.out.println("Successfully updated parent category and child categories (if needed)");
                  return true;
              } else {
                  conn.rollback();
                  return false;
  ...
  /**
   * Update parent category status only (separate method)
   */
  public boolean updateCategoryParentStatus(int parentId, boolean activeFlag) {
      Connection conn = null;
      try {
          conn = new Context().getJDBCConnection();
          ...
          String updateParentSql = "UPDATE category_parent SET active_flag = ? WHERE id = ?";
          ...
          try (PreparedStatement ps = conn.prepareStatement(updateParentSql)) {
              ...
              if (rowsAffected > 0) {
                  // 2. If parent is being deactivated, deactivate all child categories
                  if (!activeFlag) {
                      boolean childrenUpdated = deactivateChildCategories(conn, parentId);
                      if (!childrenUpdated) {
                          conn.rollback();
                          System.out.println("Failed to update child categories");
                          return false;
                  ...
                  // Commit transaction
                  conn.commit();
                  System.out.println("Successfully updated parent category status and child categories (if needed)");
                  return true;
              } else {
                  conn.rollback();
                  return false;
          ...
          System.out.println("Error when updating category parent status: " + e.getMessage());
          e.printStackTrace();
          return false;
      } finally {
          try {
   ...
   */
  private boolean deactivateChildCategories(Connection conn, int parentId) {
      String sql = "UPDATE category SET active_flag = 0 WHERE parent_id = ? AND active_flag = 1";
      
      try (PreparedStatement ps = conn.prepareStatement(sql)) {
          ps.setInt(1, parentId);
          
          int rowsAffected = ps.executeUpdate();
          System.out.println("Deactivated " + rowsAffected + " child categories for parent ID: " + parentId);
          
          return true; // Return true even if no rows affected (no child categories to deactivate)
          
      } catch (SQLException e) {
          System.out.println("Error when deactivating child categories: " + e.getMessage());
          e.printStackTrace();
          return false;
      }
  }
  ...
  /**
   * Get all child categories of a parent (for verification)
   */
  public List<Integer> getChildCategoryIds(int parentId) {
      ...
      try (Connection conn = new Context().getJDBCConnection();
          ...
          System.out.println("Error when getting child category IDs: " + e.getMessage());
          e.printStackTrace();
      }
