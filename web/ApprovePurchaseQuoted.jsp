...
<html lang="vi">
    ...
    <body>
                ...
                <table class="requests-table">
                    ...
                    <tbody id="requestsTableBody">
                        <c:choose>
                            <c:when test="${not empty purchaseOrders}">
                                <c:forEach var="po" items="${purchaseOrders}" varStatus="loop">
                                    ...
                                    <tr class="${rowClass}">
                                        ...
                                        <td>${po.fullname}</td>
                                        <%-- ✅ THÊM cột Phòng ban --%>
                                        <td>${empty po.department ? 'Chưa có' : po.department}</td>
                                        <td>
                                    ...
                                    <%-- Detail row - cập nhật colspan từ 8 thành 9 --%>
                                    <tr class="detail-row">
                                        <td colspan="9">  <%-- ✅ THAY ĐỔI từ colspan="8" thành colspan="9" --%>
                                            ...
                                            <%-- Phần hiển thị lý do từ chối giữ nguyên --%>
                                            ...
                                            <%-- Bảng chi tiết items giữ nguyên --%>
                            ...
                            <c:otherwise>
                                <tr>
                                    <td colspan="9" style="text-align: center; padding: 20px; color: #666;">  <%-- ✅ THAY ĐỔI từ colspan="8" thành colspan="9" --%>
        ...
        <script>
            ...
            // Hàm phê duyệt đơn mua hàng
            function approvePurchaseOrder(purchaseOrderId) {
                if (confirm('Bạn có chắc chắn muốn phê duyệt đơn mua hàng này không?')) {
                    // Tạo form để submit
                    const form = document.createElement('form');
                    form.method = 'POST';
                    form.action = 'approvepurchasequoted';
                    ...
                    // Thêm purchaseOrderId
                    const idInput = document.createElement('input');
                    idInput.type = 'hidden';
                    idInput.name = 'purchaseOrderId';
                    idInput.value = purchaseOrderId;
