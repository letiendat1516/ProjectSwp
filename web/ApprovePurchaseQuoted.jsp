...
<body>
  <div class="container">
      ...
      <form action="approvepurchasequoted" method="post" class="filter-section" id="filterForm">
          ...
          <div class="filter-item">
              ...
              <select id="statusFilter" name="statusFilter">
                  ...
                  <option value="rejected" ${param.statusFilter == 'rejected' ? 'selected' : ''}>Đã từ chối (chờ báo lại)</option>
                  <option value="pending" ${param.statusFilter == 'pending' ? 'selected' : ''}>Chờ duyệt</option>
              </select>
          </div>
          <div class="filter-item">
              <label for="orderIdFilter">ID Đơn hàng:</label>
              <input type="text" id="orderIdFilter" name="orderIdFilter" value="${param.orderIdFilter}" placeholder="Nhập ID đơn hàng">
          </div>
          ...
          <tbody id="ordersTableBody">
              <c:choose>
                  <c:when test="${not empty purchaseOrders}">
                      <c:forEach var="order" items="${purchaseOrders}" varStatus="loop">
                          <c:set var="status" value="" />
                          <c:set var="rowClass" value="" />
                          <c:choose>
                              <c:when test="${order.status == 'pending'}">
                                  <c:set var="status" value="Chờ duyệt" />
                              </c:when>
                              ...
                              <c:when test="${order.status == 'rejected'}">
                                  <c:set var="status" value="Đã từ chối (chờ báo lại)" />
                          ...
                          <tr class="${rowClass}">
                              ...
                              <td>
                                  <c:if test="${not empty order.dayPurchase}">
                                      <fmt:formatDate value="${order.dayPurchase}" pattern="dd/MM/yyyy" />
                                  </c:if>
                              </td>
                              <td>${status}</td>
                              ...
                              <td>
                                  <div class="action-buttons-cell">
                                      <!-- Nút xóa -->
                                      <form action="${pageContext.request.contextPath}/approvepurchasequoted" method="post" style="display:inline;">
                                          ...
                                          <input type="hidden" name="orderIdFilter" value="${param.orderIdFilter}">
                          ...
                          <tr class="detail-row">
                              <td colspan="8">
                                  <div style="margin-bottom: 10px;">
                                      ...
                                      <strong>Tên:</strong> ${order.supplier != null ? order.supplier : "Chưa có"}<br>
                                      <strong>Địa chỉ:</strong> ${order.address != null ? order.address : "Chưa có"}<br>
                                      <strong>SĐT:</strong> ${order.phone != null ? order.phone : "Chưa có"}<br>
                                      <strong>Email:</strong> ${order.email != null ? order.email : "Chưa có"}<br>
                                      <strong>Ghi chú:</strong> ${order.summary != null ? order.summary : "Không có"}
                                  </div>
                                  <table class="detail-table">
                                      ...
                                      <tbody>
                                          <c:choose>
                                              <c:when test="${not empty order.purchaseItems}">
                                                  ...
                                                  <tr style="background-color: #e9ecef; font-weight: bold;">
                                                      ...
                                                      <td class="price-cell" style="font-size: 16px; color: #dc3545;">
                                  ...
                                  <c:if test="${order.status == 'pending'}">
                                      <div class="action-buttons">
                                          <form action="${pageContext.request.contextPath}/approvepurchasequoted" method="post" style="display:inline;">
                                              ...
                                              <button type="submit" class="approve-btn" onclick="return confirm('Bạn có chắc muốn phê duyệt đơn hàng này?')">Phê duyệt</button>
                                          </form>
                                          <form action="${pageContext.request.contextPath}/approvepurchasequoted" method="post" style="display:inline;">
                                              <input type="hidden" name="orderId" value="${order.id}">
                                              <input type="hidden" name="action" value="reject">
                                              <button type="submit" class="reject-btn" onclick="return confirm('Bạn có chắc muốn từ chối đơn hàng này?')">Từ chối</button>
                                          </form>
                                      </div>
                                  </c:if>
                                  <c:if test="${order.status != 'pending'}">
                                      <div class="status-message">Đơn hàng đã được xử lý</div>
                                  </c:if>
                              </td>
                          </tr>
                      </c:forEach>
                  ...
                  <c:otherwise>
                      <tr>
                          <td colspan="8">Không có đơn hàng nào trong khoảng thời gian hoặc trạng thái này.</td>
                      </tr>
                  </c:otherwise>
              </c:choose>
          </tbody>
      ...
      <div class="pagination">
          ...
          <c:if test="${not empty param.orderIdFilter}">
              <c:set var="filterParams" value="${filterParams}&orderIdFilter=${param.orderIdFilter}" />
          </c:if>
          <c:choose>
              ...
              <c:otherwise>
                  <a href="approvepurchasequoted?index=${currentPage - 1}${filterParams}" class="pagination-button">
                      <button>
                          <span class="material-icons">chevron_left</span>
                      </button>
                  </a>
  ...
  <script>
      ...
      function clearFilters() {
          ...
          // Gửi request để hiển thị tất cả đơn hàng
          window.location.href = 'approvepurchasequoted';
      }
  </script>
</body>
</html>